...
useEffect(()=>{
  const fetch=async()=>{
    ...
    let d=await axios.get("http://localhost:5000/api/auth/getuser",option)
    datas(d.data)
    lazys(true)
...
const eref=useRef(data.email);
  ...
  await axios.delete("http://localhost:5000/api/auth/delete",options)
...
if(formdata!=null){
const d=await fetch("http://localhost:5000/api/upload",{
method:'POST',
body:formdata
}).then((da)=>{
da.json().then(e=>{
...
if(password===false){
  ...
  const data=await axios.get(`http://localhost:5000/api/auth/updateaccount`,options)
  window.location.reload()
}
    ...
    <>
        ...
        <div className='maincontainer' style={{display:"flex",flexDirection:"row"}}>
            <div className="settings" >
                ...
                <img className='Userprofile' style={{width:"100px",height:"100px",objectFit:"cover",borderRadius:"10%"}} src={file.length==0?'https://th.bing.com/th?id=OIP.oteZPbPOYq2xgDC-evNSYAHaE8&w=306&h=204&c=8&rs=1&qlt=90&o=6&dpr=1.3&pid=3.1&rm=2':`http://localhost:5000/image/${file}`} alt='No'></img>
