...
const titlechange=(e)=>{
  ...
  const fetch=await axios.delete(`http://localhost:5000/api/auth/deletepost/${id}`,options)
}
...
const editnote=async ()=>{const options1={'authtoken':localStorage.getItem('auth-token'),'Content-Type':'application/json','title':cref}
 await fetch(`http://localhost:5000/api/auth/updatepost/${id}`,{
    method:"PUT",
    headers:options1,
    body:JSON.stringify({
      title:cref,
...
const deleteitem=async ()=>{
  ...
  useEffect(()=>{setname(localStorage.getItem('name'))
   ...
   const post1=await  axios.get(`http://localhost:5000/api/auth/singlepost/${id}`,options)
  posts(post1.data[0])
  arrays(post1.data)
   console.log(post1)
   const ei=[];
